...
│   ├── 一方塊開叫.xlsx / 一梅花開叫.xlsx / 一梅花被插叫.xlsx / 二梅花開叫.xlsx /
│   │   高花開叫.xlsx / 一無王開叫.xlsx
...
3. `build_xlsx_page()` × 5 — 每一份「叫序資料」頁面一次呼叫，內含多個工作表分頁籤。
   `xlsx_name` 參數可以是單一檔名，也可以是檔名列表——列表時會把多份 xlsx 的
   工作表依序接在同一個頁面裡（一梅花開叫 = `一梅花開叫.xlsx` + `一梅花被插叫.xlsx`，
   見第 7 節「合併多份 xlsx 到同一頁」）。
...
**合併多份 xlsx 到同一頁**（例如把「1♣ 被插叫後續」併進「一梅花開叫」頁）：
`build_xlsx_page()` 的第一個參數改傳檔名列表即可，工作表會依列表順序接在一起，
分頁籤位置與 `sheet-N` id 都以合併後的順序重新編號。**但表頭列的數字編號
（如「4.A分支」「9.F-J分支」的開頭數字）必須跨檔案接續、不能重複**——
`num_to_index` 是用工作表標題開頭的數字對到分頁位置，兩份 xlsx 各自從 1
或 2 編號的話會互相蓋掉，「表4」這類跳轉連結就會連到錯的分頁。做法：合併
前先把新檔案裡有編號的工作表標題手動改成接續號碼（例如原檔到 13，新檔的
「2.LHO賭倍後」改成「14.LHO賭倍後」），沒有編號的「說明」頁則改個不會撞名
的標題（如「被插叫說明」），確認 `analyze_sheet()` 抓到的表頭列沒有跑掉
再存檔（用 `openpyxl.load_workbook()` 讀入、改 `ws.title`、`wb.save()` 即可，
不影響儲存格樣式）。
